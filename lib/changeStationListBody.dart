...
class ChangeStationList extends StatelessWidget {
  ChangeStationList({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    int numOfOption = returnOptionSize(workOrderInfo.process_options);
    ...
    final ValueNotifier<List<bool>> optionsSelected = ValueNotifier(
        List<bool>.generate(
            workOrderInfo.process_options.length, (int index) => false));
...
//Loading counter value on start
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      ...
                      if (selectList.isEmpty) {
                        showAlertDialog(
                            navigator, setPop1AlertDialog(navigator, "請選擇製程"));
                      } else if (selectList.length == 1) {
                        workOrderInfo.setSelectList(selectList);
                        showAlertDialog(
                            navigator,
                            setPop2AlertDialog(navigator,
                                "選擇的製程：${selectList[0]['process_name']}"));
                      } else {
                        showAlertDialog(
                            navigator, setPop1AlertDialog(navigator, "僅單選"));
                      }
                    },
                    style: ElevatedButton.styleFrom(
  ...
  void selectionIndex(
    ...
    final pList = workOrderInfo.process_list;
    selectList.clear();
    for (int i = 0; i < input.value.length; ++i) {
      if (input.value[i]) {
        selectList.add(pList[i]);
      }
    }
  }
