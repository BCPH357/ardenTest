          ...
          child: ElevatedButton.icon(
            onPressed: () async {
              final checkRes = await dartHttpUtils.doCheckBeforeAction(
                  ...
                  loginInfo);
              if (checkRes == 'success') {
                final res = await dartHttpUtils.inAction(
                    navigator, workOrderInfo, loginInfo);
                if (res == 'success') {
                  showAlertDialog(navigator,
                      setDoAgainAlertDialog(navigator, '進站成功', _scan));
                } else {
                  showAlertDialog(
                ...
                if (res == 'success') {
                  showAlertDialog(
                      ...
                      reInputOrderIDAlertDialog(
                          '進站成功', navigator, workOrderInfo, loginInfo, '進站'));
                } else {
                  showAlertDialog(
                      navigator, setPop1AlertDialog(navigator, res));
                }
      ...
      if (scanResult != null && scanResult?.rawContent != "") {
        if (result.format.toString() == 'qr') {
          ...
          final res = await dartHttpUtils.reDoCheck(scanResult?.rawContent,
              '進站', navigator, workOrderInfo, loginInfo);
          if (res != 'success') {
...
class InStation extends StatelessWidget {
  const InStation({super.key});
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      if (checkRes == 'success') {
                        ...
                        if (res == 'success') {
                          showAlertDialog(navigator,
                              setPop3AlertDialog(navigator, '進站成功'));
                        } else {
                          showAlertDialog(
                              navigator, setPop1AlertDialog(navigator, res));
                        }
